...
// Select UI components removed (Create Application card removed)
...
// app form type removed (Create Application UI removed)
...
export default function AdminPage() {
  ...
  const handleDeleteSelectedStaff = async () => {
    ...
    try {
      ...
      await Promise.all(
        ids.map((id) => remove(ref(realtimeDb, `${STAFF_COLLECTION}/${id}`)))
      )
      setSelectedStaff(new Set())
  ...
  // Create application UI removed; creation handled elsewhere if needed
  ...
  if (authLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center text-sm text-gray-600">Checking admin session...</div>
    )
  }
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {/* Create Application card removed */}
      </div>
