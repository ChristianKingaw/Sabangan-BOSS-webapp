...
const MHO_USERS_PATH = "users/webapp/mho"
...
const findMhoRecordByEmail = async (email: string) => {
  ...
  const findByScan = async () => {
    const fullSnapshot = await adminDb.ref(MHO_USERS_PATH).get()
...
export async function GET(request: NextRequest) {
  try {
    ...
    const record = await findMhoRecordByEmail(userEmail)
    if (!record) {
      return NextResponse.json(
        { error: "Authenticated user is not an MHO account." },
        { status: 403 }
      )
    }
    ...
    console.error("Failed to validate MHO auth profile", error)
    return NextResponse.json(
      { error: "Unable to validate MHO account." },
      { status: 500 }
    )
  }
}
